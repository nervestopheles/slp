void CalcOfSets()
{
    ...
    if (compFlag == _compXinY)
        compStr = "Set V(capacity) is contained in set M(mass).";
    else if (compFlag == _compYinX)
        compStr = "Set M(mass) is contained in set V(capacity).";
    else if (compFlag == _compEqual)
        compStr = "Sets are equal.";
    else 
        compStr = "Nothing.";
    printf("  %s\n", compStr);
}